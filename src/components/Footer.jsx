...
const Footer = () => {
    ...
    const [footerColor, setFooterColor] = useState('#FCE7F3');

    useEffect(() => {
        setFooterColor(pinkinator({ intensity: 150 }));
    }, []);
    ...
    return (
        <footer className="text-center text-surface/75 dark:text-white/75 lg:text-left">
            ...
            <div className="py-3 text-xs text-gray-400 text-center">
                <a
                    href="https://www.npmjs.com/package/pinkinator"
                    target="_blank"
                    rel="noopener noreferrer"
                    className="inline-flex items-center gap-1.5 hover:text-gray-600 transition-colors"
                >
                    <span className="w-2 h-2 rounded-full inline-block" style={{ backgroundColor: footerColor }} suppressHydrationWarning />
                    Powered by pinkinator
                </a>
            </div>
        </footer>
    )
